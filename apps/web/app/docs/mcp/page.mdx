...
There are two ways to connect, with the same sixteen tools either way:
...
After the client reloads, it should list **sixteen** tools. Ask it to enumerate
...
year), and `firdaria` (the planetary time-lord periods).
...
If you see all sixteen, the server is wired correctly. The server also exposes two
