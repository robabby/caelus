...
There are two ways to connect, with the same twenty-eight tools either way:
...
After the client reloads, it should list **twenty-eight** tools. Ask it to enumerate
...
void-of-course Moon, with no birth chart needed), and `chart_facts` (the
chart's validated facts as ranked, citable atoms plus a ready-to-interpret
brief, so the host model writes a reading grounded in correct math and cites
the fact behind each claim — see the
[Interpretation guide](/docs/interpretation)).
...
If you see all twenty-eight, the server is wired correctly. The server also exposes two
...
<CodeBlock
  ...
  "aspects": [
    { "a": "sun", "b": "moon", "aspect": "quincunx", "orb": 3.8, "phase": "applying", "strength": 0.62 }
  ]
}`}
/>
