...
There are two ways to connect, with the same twenty-seven tools either way:
...
After the client reloads, it should list **twenty-seven** tools. Ask it to enumerate
...
and `cosmic_weather` (the day's active configurations, stations, and
void-of-course Moon, with no birth chart needed).
...
If you see all twenty-seven, the server is wired correctly. The server also exposes two
