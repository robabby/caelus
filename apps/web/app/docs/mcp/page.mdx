...
There are two ways to connect, with the same seventeen tools either way:
...
After the client reloads, it should list **seventeen** tools. Ask it to enumerate
...
year), `firdaria` (the planetary time-lord periods), and `releasing` (zodiacal
releasing / aphesis from the Lot of Spirit or Fortune).
...
If you see all seventeen, the server is wired correctly. The server also exposes two
