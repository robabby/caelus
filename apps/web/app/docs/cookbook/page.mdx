...
<CodeBlock
  lang="typescript"
  label="dignity-score.ts"
  code={`import { dignityScore, almuten } from "caelus";
...
<CodeBlock
  lang="typescript"
  label="parans.ts"
  code={`import { parans } from "caelus";
...
- **`similar_skies`**, **`electional_search`**, **`cosmic_weather`**: when the
  sky last resembled a moment, the best window for a set of aspects, and a
  date's active configurations and stations.
