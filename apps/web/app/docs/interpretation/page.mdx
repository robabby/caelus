...
**`InterpretationSource`** bundles rules — a tradition, a house style, a
...
tool, for hosts where the model is already the interpreter (Claude, Cursor, …).
It returns a chart's ranked, citable atoms plus a ready `brief`, so the host
interprets from correct math and cites the `[id]` each statement rests on
instead of re-deriving — and hallucinating — positions.
