...
- **`enrichContextOptions(engine, chart, target)`** — transits to natal (with
  phase and strength), active profection / zodiacal releasing / firdaria /
  Vimshottari dasha, Egyptian term/face/triplicity/almuten facts (always), and
  nakshatra / D9 / yogas when the chart is sidereal.
- **`enrichSynastryOptions(engine, chartA, chartB)`** — inter-chart aspects,
  house overlays, and composite midpoint placements (chart A is the base).
