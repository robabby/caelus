...
facets add `facet`/`value`; dispositors add `dispositor`/`final`.
...
<CodeBlock
  ...
  hasDispositor, hasReception, matchAll, matchAny, matchNone,
} from "caelus";
...
on instead of re-deriving (and hallucinating) positions.
